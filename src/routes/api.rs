...
#[tracing::instrument(skip_all)]
async fn create_asset(_admin : Admin, repository : Repository, Json(request) : Json<CreateAssetRequest>) -> Result<Json<Asset>, AppError>{
    
    if request.name.is_empty() || request.unit_value <= 0.0 { 
        return Err(AppError::AssetInvalidData)
    }

    match repository.find_asset_by_name(&request.name).await? { 
        Some(_) => return Err(AppError::AssetAlreadyExist),
        None => {
            let new_asset = repository.create_asset(request.name, request.unit_value).await?;
            return Ok(Json(new_asset))},
    };
...
#[tracing::instrument(skip_all)]
async fn update_asset(_admin : Admin, repository : Repository, Json(request): Json<UpdateAssetRequest>,) -> Result<Json<Asset>, AppError>{
...
use super::*;
    ...
    async  fn test_create_asset(db : PgPool){
        let request = CreateAssetRequest{name : "Bitcoin".to_string(), unit_value: 10.0};
    ...
    async  fn test_update_asset(db : PgPool){
        let request = UpdateAssetRequest{id : 1, name : Some("Ethereum".to_string()), unit_value: Some(20.0)};
