...
use tokio::try_join;
use crate::{app::AppState, auth::user::{UnauthenticatedUser, User}, error::AppError, models::{Asset, OwnedAsset}, repository::Repository};
...
async fn login(repository : Repository, jar:CookieJar, Form(request) : Form<LoginForm>) -> Result<impl IntoResponse, AppError> {
...
pub async fn assets(repository : Repository, user : User) -> Result<Html<String>, AppError>{ 
    ...
    let html = AssetsPage {
        owned_assets, available_assets, user
    }.render()?;
    Ok(Html(html))
}
...
pub async fn purchase_asset(repository : Repository, 
    user : User, 
    Form(request) : Form<PurchaseAssetForm>
) -> Result<Redirect, AppError> {
    repository.insert_owned_assets(
        user.id(),
        request.asset_id,
        request.quantity,
        request.unit_value ).await?;
...
pub mod filters {
    ...
    pub fn human_datetime(
        ...
        const HUMAN_READABLE_FORMAT: StaticFormatDescription = format_description!(version = 2, "[year]-[month]-[day] [hour]:[minute]");

        datetime.format(HUMAN_READABLE_FORMAT).map_err(askama::Error::custom)
    }
}