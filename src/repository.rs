...
impl Repository {
    ...
    pub async  fn update_asset(&self, id : i64, name : Option<String>, unit_value : Option<f64>) -> sqlx::Result<Option<Asset>>{
        sqlx::query_as!(
            Asset,
            "UPDATE assets 
                SET name = COALESCE($2, name), unit_value =COALESCE($3, unit_value) 
                WHERE id = $1
                RETURNING id, name, unit_value;",
                id, name, unit_value
    ...
    pub async fn list_owned_assets(&self, user_id : i64) -> sqlx::Result<Vec<OwnedAsset>> {
        ...
        pub async fn insert_owned_assets(&self, user_id : i64, asset_id: i64, quantity: f64, unit_value: f64) -> sqlx::Result<()> {
           sqlx::query!(
        ...
        pub async fn find_asset_by_name(&self, asset_name : &str) -> sqlx::Result<Option<Asset>> {
            sqlx::query_as!(
                Asset,
                "SELECT * FROM assets 
                    WHERE name = $1",
